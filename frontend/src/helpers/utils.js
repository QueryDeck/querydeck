...
export const isSubdomainIsSandbox = subdomain => {
  return subdomain === 'sandbox'
};

export const textSlicer = (text, maxLength, options = { seperator: null, }) => {

  let endString  = ''
  if (options.seperator) {
    text = text.split(options.seperator) 
    if( text.length > 1){ 
      endString = '...'
    }
    text = text[0]
  ...
  return `${text.slice(0, maxLength)}${endString}`;
